		...
		<view :options="options" :type="type" :color="color" :strokeWidth="strokeWidth" :change:color="draw.setColor"
			...
			<view class="right-tools">
				<template v-for="(items, index) in toolsConfig" :key="index">
					<text :style="{ color: type == items.tool ? 'rgb(49, 76, 215)' : '' }"
						@click="drawActon(items.tool)" class="clear">{{ items.title }}</text>
				</template>
...
<script module="draw" lang="renderjs">
	import konva from "@/libs/konva.js"
	export default {
		mounted() {
			if (this.options && this.options.width != 0 && this.options.height != 0) {
				this.initKonva();
        	}
		},
		data() {
