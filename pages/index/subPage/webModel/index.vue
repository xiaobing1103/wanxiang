<template>
	<z-paging :show-scrollbar="false" ref="srollRef"
		...
		<template #top>
			<CommonHeader defindTitle="AI网页模型" />
		</template>
						...
						<view class="voiceLink_uploadFile">
							<up-upload :customStyle="{ width: '100%' }" :accept='accept' :previewImage="false"
								...
								<view class="voiceLink_uploadFile_box">
									<image class="voiceLink_uploadFile_imageIcon" :src="$assets.webModelIcon" mode="">
									</image>
									<text class="voiceLink_uploadFile_topText">点击此区域进行文件上传</text>
									<text class="voiceLink_uploadFile_bottomText">您可以上传.html、.htm 格式的文件</text>
								</view>
							</up-upload>
						</view>
					</view>
...
<script setup lang="ts">
	...
	const startFast = async () => {
		...
		if (isPureLink(linkVal.value)) {
			resvideo = await $api.get('api//v1/media/gethtml', { url: linkVal.value })
			if (resvideo.code == 200) {
				onSend('请理解我发送的网页链接', null, [{ role: 'user', content: '我的内容是：' + resvideo.data }])
			} else {
				uni.$u.toast(resvideo.msg);
				showChatBox.value = false
			}
		} else {
			uni.$u.toast('只支持http:// 或者 https://  的链接！');
			showChatBox.value = false
			...
			// 	if (res.code == 200) {
			// 		onSend('请理解我发送的网页链接', null, [{ role: 'user', content: '我的内容是：' + res.text }])
			// 	} else {
			// 		uni.$u.toast(res.msg);
			// 		showChatBox.value = false
			// 	}
	...
	const afterRead = async (event : { file : any; }) => {
		...
		if (!checkFileType(fileName)) {
			uni.$u.toast('只支持上传文件格式.htm, .html！');
			return
		}
		showChatBox.value = true
		// #ifdef MP-WEIXIN
	...
	const descriptionImage = async (formData : string) => {
		let res : any
		// #ifdef H5
		res = await $api.post('api/v1/media/readhtml', formData)
		// #endif
		// #ifdef MP-WEIXIN
		res = await $api.post('api/v1/media/readhtml', formData, true, {}, null, true)
		// #endif
	...
	const onSend = async (
		...
		const reqData = {
			prompt: `我希望你根据我提供的网页内容进行总结，并以此网页内容来回答问题}`,
