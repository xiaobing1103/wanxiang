			...
			<z-paging ref="pagingRef2" :pagingStyle="{background:'rgb(246, 247, 249)'}">
				...
				<template #bottom>
					<view class="overlay_warp_footer">
						<u-button :disabled="isRecive" @click="showModal = true"
							:custom-style="{height: '60rpx' , margin: '40rpx' , padding: '20rpx 0' }"
							type="error">删除</u-button>
						<u-button :disabled="isRecive"
							:custom-style="{height: '60rpx' , margin: '40rpx' , padding: '20rpx 0' }"
							@click="createDaGang" type="primary">开始</u-button>
						<u-button :disabled="isRecive" @click="resetCreateDaGang"
							:custom-style="{height: '60rpx' , margin: '40rpx' , padding: '20rpx 0' }"
							type="default">重试</u-button>
						<u-button :disabled="isRecive"
							:custom-style="{height: '60rpx' , margin: '40rpx' , padding: '20rpx 0' }"
							@click="saveDaGang " type="warning">
							保存
						</u-button>
					</view>
				</template>
...
`
		...
		streamRequest({
			...
			onfinish() {
				// 使用正则表达式匹配章节标题和内容
				const chapterRegex = /(一|二|三|四|五|六|七|八|九|十)、[^\n]+(?:\n+(?![一二三四五六七八九十百千万]+、)[^\n]+)*/g;
				// 找到所有章节
				...
				// 将格式化后的章节放入数组
				const resultArray = formattedChapters;
				// 输出结果
				if (resultArray.length > 0) {
					resultArray.forEach((items) => {
						outlines.value.push({ title: items, content: '', complete: false })
					})
				}
				contentStr.value = ''
			},
