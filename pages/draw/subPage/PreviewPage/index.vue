<template>
	<z-paging ref="srollRef" :pagingStyle="{ background: 'rgb(246, 247, 249)', padding: '0 30rpx' }">
		<template #top>
			<CommonHeader :backPageNum="2" :defindPath="drawProjectConfig[drawStore.seletedDrawProject].path" />
		</template>
		<view class="body">
			<view class="PreviewPage">
				...
				<view class="PreviewPage_content">
					<uni-swiper-dot :style="{ height: '100%', width: '100%' }" :info="currentImages?.images" :current="current" field="content" :mode="'round'">
						<view class="swiper">
							<swiper class="swiper-box" @change="change">
								<swiper-item class="swiper-item" v-for="(item, index) in currentImages?.images" :key="index">
									<view class="swiper-item">
										<image class="swiper-item_image" :src="item" mode="aspectFill" @click="previewImage(item)"></image>
									</view>
								</swiper-item>
							</swiper>
						</view>
		...
		<template #bottom>
			<view class="PreviewPage_footer">
				...
				<view class="PreviewPage_footer_handlers">
					<view class="PreviewPage_footer_handlers_items" @click="toDownload(currentImages?.images[current], '1')">
