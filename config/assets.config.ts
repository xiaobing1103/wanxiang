
export interface IconConfig {
	bianJieVipCard : string;
	wanXiangVipCard : string;
	bianJieNoVipCard : string;
	wanXiangNoVipCard : string;
	ppt_cover : string;
	aixz_cover : string;
	gzzj_cover : string;
	swdt_cover : string;
	wzyh_cover : string;
	wddh_cover : string;
	xgst_cover : string;
	tpzkt_cover : string;
	aihl_cover : string;
	paintAixz_cover : string;
	tpst_cover : string;
	wzst_cover : string;
	jbzh_cover : string;
	rxkt_cover : string;
	AishaShiIcon : string
	v35ModelIcon : string;
	v40ModelIcon : string;
	vfastModelIcon : string;
	netModelIcon : string;
	Change_Model_Icon : string
	echartsModelIcon : string
	glmModelIcon : string
	baiduModelIcon : string
	baidu40ModelIcon : string
	zhipuModelIcon : string
	zhipu_glm4Icon : string
	tongyiIcon : string
	kimiIcon : string
	baidu_llamaIcon : string
	tongyiMaxIcon : string
	img2cartoonIcon : string
	AiChangeFace : string
	/**
	 * 文档理解模型icon
	*/
	docModelIcon : string
	imageModelIcon : string
	voiceModelIcon : string
	phoneIcon : string
	emailIcon : string
	userIcon : string
	qqIcon : string
	weixinIcon : string
	miyaoIcon : string
	resetPassWord : string
	deleteAccount : string
	emailChangeIcon : string
	gongZhongHao : string
	resetPassWordLock : string
	defineAvatar : string
	weiXinQun : string
	weiXinKeFu : string
	tuYaDrawIcon : string
	tuYaDrawTiaoseIcon : string
	uploadImageIcon : string
	previewImageIcon : string,
	videoModelIcon : string
	webModelIcon : string
	chat40ModelIcon : string
	codegeex_4ModelIcon : string
	pptIcon : string
	/**
	 * 图片反推提示词icon
	*/
	reserveWordIcon : string
	/**
	 * 图片反推提示词 头部上传图标 icon
	*/
	reserveWordHeaderIcon : string
	/**
	 * 图片反推提示词 推演结果图标 icon
	*/
	reserveContentIcon : string
	/**
	 * 图片反推提示词 估值结果图标 icon
	*/
	reserveCopyContentIcon : string
	/**
	 * vip砖石图标 icon
	*/
	vipIcon : string
	/**
	* 支付宝图标 icon
	*/
	zhiFuBaoIcon : string
	/**
	* 微信图标 icon
	*/
	weChatIcon : string
	/**
	* 重新加载 icon
	*/
	reloadIcon : string
	/**
	* 皇冠 icon
	*/
	huangGuanIcon : string
	/**
	* 公文写作 icon
	*/
	officeWriteIcon : string
	/**
	* 论文写作 icon
	*/
	artistWriteIcon : string
	AiVioceCreateIcon : string
	OrcScannerIcon1 : string
	OrcScannerIcon2 : string
	OrcScannerIcon3 : string
	OrcScannerIcon4 : string
	OrcScannerIcon5 : string
	ORCPageIcon : string
	ArtistDownIdon : string
	AImappingPreview : string
	noData : string
	TranslatePagesIcon : string
	AiTranslateIcon : string
	AiVioceTrasnlate : string
	AIWriteSomething : string
}
const APP_CONFIG = {
	staticPath: 'http://file.1foo.com',
}
const getUrl = (url : string) => {
	return APP_CONFIG.staticPath + url
}

// types.ts
export interface IconConfig {
	bianJieVipCard : string;
	wanXiangVipCard : string;
	bianJieNoVipCard : string;
	wanXiangNoVipCard : string;
	ppt_cover : string;
	aixz_cover : string;
	gzzj_cover : string;
	swdt_cover : string;
	wzyh_cover : string;
	wddh_cover : string;
	xgst_cover : string;
	tpzkt_cover : string;
	aihl_cover : string;
	paintAixz_cover : string;
	tpst_cover : string;
	wzst_cover : string;
	jbzh_cover : string;
	rxkt_cover : string;
	AishaShiIcon : string
	v35ModelIcon : string;
	v40ModelIcon : string;
	vfastModelIcon : string;
	netModelIcon : string;
	Change_Model_Icon : string
	echartsModelIcon : string
	glmModelIcon : string
	baiduModelIcon : string
	baidu40ModelIcon : string
	zhipuModelIcon : string
	zhipu_glm4Icon : string
	tongyiIcon : string
	kimiIcon : string
	baidu_llamaIcon : string
	tongyiMaxIcon : string
	img2cartoonIcon : string
	AiChangeFace : string
	/**
	 * 文档理解模型icon
	*/
	docModelIcon : string
	imageModelIcon : string
	voiceModelIcon : string
	phoneIcon : string
	emailIcon : string
	userIcon : string
	qqIcon : string
	weixinIcon : string
	miyaoIcon : string
	resetPassWord : string
	deleteAccount : string
	emailChangeIcon : string
	gongZhongHao : string
	resetPassWordLock : string
	defineAvatar : string
	weiXinQun : string
	weiXinKeFu : string
	tuYaDrawIcon : string
	tuYaDrawTiaoseIcon : string
	uploadImageIcon : string
	previewImageIcon : string,
	videoModelIcon : string
	webModelIcon : string
	chat40ModelIcon : string
	codegeex_4ModelIcon : string
	pptIcon : string
	/**
	 * 图片反推提示词icon
	*/
	reserveWordIcon : string
	reserveWordHeaderIcon : string
	reserveContentIcon : string
	/**
	 * 图片反推提示词 估值结果图标 icon
	*/
	reserveCopyContentIcon : string
	/**
	 * vip砖石图标 icon
	*/
	vipIcon : string
	/**
	* 支付宝图标 icon
	*/
	zhiFuBaoIcon : string
	/**
	* 微信图标 icon
	*/
	weChatIcon : string
	/**
	* 重新加载 icon
	*/
	reloadIcon : string
	/**
	* 皇冠 icon
	*/
	huangGuanIcon : string
	/**
	* 公文写作 icon
	*/
	officeWriteIcon : string
	/**
	* 论文写作 icon
	*/
	artistWriteIcon : string
	AiVioceCreateIcon : string
	OrcScannerIcon1 : string
	OrcScannerIcon2 : string
	OrcScannerIcon3 : string
	OrcScannerIcon4 : string
	OrcScannerIcon5 : string
	ORCPageIcon : string
	ArtistDownIdon : string
	oldTranslateIcon : string
	AiImageTranslate : string
	AiImageExampleImage : string
	ArticleTranslate : string
	ArticleUserIcon : string
	wordFileIcon : string
	ppxtRedFileIcon : string
	excelFileIcon : string
	ppxtYellowFileIcon : string
	lunWenXueShuTranslate : string
	enXueShuTranslate : string
	AImusicBackIcon : string
	AIMusicLogoIcon : string
	AIZidongCreateSong : string
	AIMyCreateSong : string
	todayIcon : string
	zhangJia : string
	zhangzhi : string
	MusicSquare : string
	ImageCreateMusicIcon : string
	VideoCreateMusicIcon : string
	doubao_llamaIcon : string
	text2videoIcon : string
	createVideoVipIcon : string
	wechatPayIcon : string
	alipayIcon : string
	image2videoIcon : string
	AINumersPeopleBgImage : string
	AIVideoHandlerImageBgImage : string
	AIExtractMusicImageBgImage : string
	AIThesamePointIcon : string
	InspiredWritingIcon : string
	articleWritingIcon : string
	createTitleIcon : string
	behindQrcodeIcon : string
	AIzhaiYaoIcon : string
	editIcon : string
	articleQRcodoeIcon : string
	aiAgentheaderModeIcon1 : string
	aiAgentheaderModeIcon2 : string
	aiAgentheaderModeIcon3 : string
	aiAgentheaderModeIcon4 : string
	aiAgentheaderModeIcon5 : string
	aiAgentheaderModeIcon6 : string
	aiAgentheaderModeIcon7 : string
	aiAgentheaderModeIcon8 : string
	aiAgentheaderModeIcon9 : string
	aiAgentheaderModeIcon10 : string
	aiAgentheaderModeIcon11 : string
	aiAgentheaderModeIcon12 : string
	aiAgentheaderModeIcon13 : string
	aiAgentheaderModeIcon14 : string
	aiAgentheaderModeIcon : string
	uploadFileIcon : string
	aiAigentLoadingIcon : string
	aiAigentLoadingImage : string
	aiAigentDrawLoading : string
	aiRevers3D : string
	translate20 : string
	aiApplication : string
	Application2 : string
	Application3 : string
	Application4 : string
	Application5 : string
	Application6 : string
	Application7 : string
	Application8 : string
	Application9 : string
	Application10 : string
	Application11 : string
	Application12 : string
	Application13 : string
	Application14 : string
	Application15 : string
	Application16 : string
	Application17 : string
	Application18 : string
	Application19 : string
	Application20 : string
	Application21 : string
	Application22 : string
	Application23 : string
	Application24 : string
	ApplicationsBackground : string
	NoApplicationsData : string
	DeepSeekModelIcon : string
	DeepSeekModalThinkIngIcon : string
	copyIcon : string
	reStartChatIcon : string
	ExportIcon : string
	jubaoIcon : string
	lianwangIcon : string
	OverBGImage : string
}
export interface ModelConfig {
	title : string;
	ModelPath : string;
	modelIcon : string;
	ModelApi : string;
	modelDesc : string;
	resetPassWord ?: string
	checkNumsType ?: string
}

export type CommonModelKeys = 'v35' | 'v40' | 'net' | 'echarts' | 'vfast';
const iconConfig : IconConfig = {
	bianJieVipCard: getUrl('/2024/10/06/1fb9a51b8bbcbf3ceb2e1428dee63bbd.png'),
	wanXiangVipCard: getUrl('/2024/07/29/396b53c790a06edd99dd4676c9066bd8.svg'),
	bianJieNoVipCard: getUrl('/2024/10/06/bb71cecc0080693eece1a6f3507c3e65.png'),
	wanXiangNoVipCard: getUrl('/2024/09/21/946bb2019074124cc1b806209fdc5d40.png'),
	ppt_cover: getUrl('/2024/07/29/ae8a249b18293065506c5c18f9bf4802.svg'),
	aixz_cover: getUrl('/2024/07/29/d43cefcff1e8a7c8c15ff4d0d3e6e391.svg'),
	gzzj_cover: getUrl('/2024/07/29/aa9686df0e44172525901ab0a23ca56d.svg'),
	swdt_cover: getUrl('/2024/07/29/55725ecb144c844178dbb1c178c75cca.svg'),
	wzyh_cover: getUrl('/2024/07/29/a17dca27807bb8f048a6c963563da670.svg'),
	wddh_cover: getUrl('/2024/07/29/23319b9224489ef905f0e677fe0d9a6e.svg'),
	xgst_cover: getUrl('/2024/07/29/b8995e5f2bca2177a828807f33e2fed6.svg'),
	tpzkt_cover: getUrl('/2024/07/29/76df995762df7fb7f831683dff8fa0d5.svg'),
	aihl_cover: getUrl('/2024/07/29/d7a6a27f4d8fdcaa941a91192a21acc9.svg'),
	paintAixz_cover: getUrl('/2024/07/29/98b5614e65a4c4383ca14cfb1a49ab39.svg'),
	tpst_cover: getUrl('/2024/07/29/9d35318e0f3a1915c1aa54dc3d62da52.svg'),
	wzst_cover: getUrl('/2024/07/29/79cf1c92dad3344d09c588ed894d15fc.svg'),
	jbzh_cover: getUrl('/2024/07/29/264f04a7b3cae3546bf0544c201d0d7c.svg'),
	rxkt_cover: getUrl('/2024/07/29/b5e3e490318dd92361e3afee5e16c0bd.svg'),
	v35ModelIcon: getUrl('/2023/12/12/941983dadec37a257e67974675d21734.png'),
	v40ModelIcon: getUrl('/2024/08/21/3095dc6c36fb1cf9d7f59f0237b4faf0.svg'),
	vfastModelIcon: getUrl('/2023/12/12/ce9426096ab671825e0ba57c750a14d8.png'),
	netModelIcon: getUrl('/2023/12/12/ce9426096ab671825e0ba57c750a14d8.png'),
	echartsModelIcon: getUrl('/2024/01/02/d4169bfc46ee0d0a29c6c9c8653ebc06.png'),
	baidu40ModelIcon: getUrl('/2024/08/21/dc0ef1c1e68965e956d47f5797e80ff8.svg'),
	glmModelIcon: getUrl('/2024/08/21/04e13289b698355ad1e76adef3ef4d83.svg'),
	tongyiIcon: getUrl('/2024/01/02/974747351ea31ff54ead4818cde5b24b.svg'),
	tongyiMaxIcon: getUrl('/2024/05/21/8816ccfb5cc11b08f140db94d8a9a187.svg'),
	baiduModelIcon: getUrl('/2024/08/21/040bdf5efc10359ba47df6f0b608ad78.svg'),
	Change_Model_Icon: getUrl('/2024/08/06/1a164d08dfe054b06dac984aecf187ad.svg'),
	kimiIcon: getUrl('/2024/08/21/0cc7bd1c98297451f0bba50da466914d.svg'),
	zhipuModelIcon: getUrl('/2024/08/21/04e13289b698355ad1e76adef3ef4d83.svg'),
	docModelIcon: getUrl('/2024/02/22/f8fdbf7ea31f0f5e772dd80f37e5c334.png'),
	zhipu_glm4Icon: getUrl('/2024/05/21/3eedb54d58430193f016a28bc0d70d9a.svg'),
	baidu_llamaIcon: getUrl('/2024/08/21/2ef38da657fe22609a2dd0ab51c2b80a.svg'),
	doubao_llamaIcon: getUrl('/2024/12/18/e0558db52ce548746df4027a9fe63f1e.svg'),
	imageModelIcon: getUrl('/2024/02/22/36128b71a39265bebeba2a1abf6484f7.png'),
	voiceModelIcon: getUrl('/2023/11/07/a74dddf2100b8d633fb0d3ea0716a2e9.svg'),
	videoModelIcon: getUrl('/2024/02/22/1c76f2f33e6ac2778e894da18b1caf22.png'),
	webModelIcon: getUrl('/2024/08/21/d375c55fc4603e3b81bcdc259c52ebf4.svg'),
	phoneIcon: getUrl('/2024/08/22/70d5a0aece7b0fb07c33ab7f0aa353ee.svg'),
	emailIcon: getUrl('/2024/08/22/7370adddc50c34d2edf3d53e5f296dad.svg'),
	userIcon: getUrl('/2024/08/22/2f4d498b3830367e37d45046f5f2f55a.svg'),
	qqIcon: getUrl('/2024/08/22/4560fb29d2d25aa6b0de77868e58af5b.svg'),
	weixinIcon: getUrl('/2024/08/22/dac0f6fc6b22a50aa1f70c9f58b7cfc3.svg'),
	miyaoIcon: getUrl('/2024/08/22/ba0406c40d7f38bd852d2da4cbfd36fd.svg'),
	resetPassWord: getUrl('/2024/08/22/db274fec18803c9d60422966b1c92d53.svg'),
	deleteAccount: getUrl('/2024/08/22/babdfe9f886b76e2fcd6958075990e44.svg'),
	emailChangeIcon: getUrl('/2024/03/12/4e02841dd340f136b52718f489c34de2.png'),
	gongZhongHao: getUrl('/2024/08/23/d5ae18cfe91fe606991ae0eff484e9d0.jpg'),
	resetPassWordLock: getUrl('/2024/03/12/ce0a123619c14302caddd7cc934c498b.png'),
	defineAvatar: getUrl('/2024/08/23/70f89ca27f79ef1ae2217960604257da.jpg'),
	weiXinQun: getUrl('/2024/08/23/96a9187a2fba2d9ddb065e63a9ef0fe3.jpg'),
	weiXinKeFu: getUrl('/2024/08/23/d3d0d0745124cb68033b27b05dc542f9.png'),
	//涂鸦的画笔图标
	tuYaDrawIcon: getUrl('/2024/03/04/d91ddc60da14ea8423beb09fa31108ab.png'),
	//涂鸦的调色盘图标 
	tuYaDrawTiaoseIcon: getUrl('/2024/08/30/de0f35a10c54aec2f376e42ead525fa9.svg'),
	//上传图片的图标
	uploadImageIcon: getUrl('/2024/03/04/8aed983f23cce83b7190c21c7f9dbef9.png'),
	//预览图片的图标
	previewImageIcon: getUrl('/2024/03/04/0e1835b5223697f669522ac8c6921088.png'),
	chat40ModelIcon: getUrl('/2024/09/11/f78dc561284062ad7b5b6f5715284987.svg'),
	codegeex_4ModelIcon: getUrl('/2024/09/11/84915a678e556e905d35d21ea0d520d2.svg'),
	reserveWordIcon: getUrl('/2024/09/18/84c560e5aec3e620d5cb6f87baa16204.png'),
	reserveWordHeaderIcon: getUrl('/2024/03/04/8aed983f23cce83b7190c21c7f9dbef9.png'),
	reserveContentIcon: getUrl('/2024/03/23/6216fc7507863a489711b31f05e5327f.svg'),
	reserveCopyContentIcon: getUrl('/2024/03/23/8e62293bb06d567ffdda5e82a599bde6.svg'),
	vipIcon: getUrl('/2023/12/27/db9bc66cc1473835b946a72697e7fc03.svg'),
	zhiFuBaoIcon: getUrl('/2024/09/19/98d77787b35159f0f719bcde29c9a89b.svg'),
	weChatIcon: getUrl('/2024/09/19/2c5d4e8a115d4ac03e09d63341095704.svg'),
	reloadIcon: getUrl('/2024/09/11/b1f6ae2fe5473c1c18323f1739dc3119.svg'),
	huangGuanIcon: getUrl('/2024/08/20/7e08b99320e97fe9a32258e53d2b0f8d.svg'),
	pptIcon: getUrl('/2024/10/03/90cb0715fe03da15be454d21f9f65ae5.svg'),
	img2cartoonIcon: getUrl('/2024/10/03/61e754c90151fc129a1d6406e21d66d7.svg'),
	AiChangeFace: getUrl('/2024/10/03/143583d04ac2c96692459ef6e77eedea.svg'),
	AishaShiIcon: getUrl('/2024/10/03/349b92f6e58d4708fd8dc93e62cf657b.svg'),
	// 公文写作
	officeWriteIcon: getUrl('/2024/10/07/3ab84da59078d8a27636a6a033fbe765.jpg'),
	// 论文写作
	artistWriteIcon: getUrl('/2024/10/07/aee46630d21e6e58220db58a5994a910.jpg'),
	// AI真人语音合成
	AiVioceCreateIcon: getUrl('/2024/10/18/7127f149b61a959667bf205f4be3246d.png'),
	// AIOrc
	ORCPageIcon: getUrl('/2024/10/19/215169639c660b763bd6b1b25a0a3357.jpg'),
	// ORC识别功能 
	OrcScannerIcon1: getUrl('/2024/10/19/c80ae5511e46f2a801c98ac68417481c.jpg'),
	OrcScannerIcon2: getUrl('/2024/10/19/29cf24d6a62ff33a69ff6f94499c98eb.jpg'),
	OrcScannerIcon3: getUrl('/2024/10/19/7231a3fb12b0247d2f838d50adc7e6ae.jpg'),
	OrcScannerIcon4: getUrl('/2024/10/19/a9a68d4b6cded3c47e5a739e82edd968.jpg'),
	OrcScannerIcon5: getUrl('/2024/10/19/f8dc0884421388814f7f789a6d8569d9.png'),
	// 论文降重
	ArtistDownIdon: getUrl('/2024/10/21/867ee284f3d2970284e486d7d1c9a012.png'),
	// 论文查重
	AImappingPreview: getUrl('/2024/10/28/7141a100497f2f0eec12c456ad3fcada.jpg'),
	noData: getUrl('/2024/10/28/85ae31614e56904cf023fccfaecf7236.png'),
	// ai翻译
	TranslatePagesIcon: getUrl('/2024/10/29/a746e7ed6080b908645ba7b670f24d02.webp'),
	// ai翻译通用
	AiTranslateIcon: getUrl('/2024/11/01/cdea132000f0e7aa4e746378b53baf01.png'),
	oldTranslateIcon: getUrl('/2024/11/01/e6b9033ede65f2a0a95672508c83a8ba.jpg'),
	AiVioceTrasnlate: getUrl('/2024/11/02/570fabaecb90e8c1342bf380ffcaab14.png'),
	AiImageTranslate: getUrl('/2024/11/02/531de01549b0056c0ba734966c8453a5.jpg'),
	AiImageExampleImage: getUrl('/2024/11/02/6b9b867d5a3711e0d43543ee618fa2b4.png'),
	ArticleTranslate: getUrl('/2024/11/04/812ba04563dea96ad5b006381199affa.png'),
	ArticleUserIcon: getUrl('/2024/10/25/7da9053a0fc370479ffd77ef8e097ce4.png'),
	wordFileIcon: getUrl('/2024/10/19/b9d1db39b6e9d44dbfdda223ba59e258.svg'),
	ppxtRedFileIcon: getUrl('/2024/10/19/fcdf9e3eb07feae1362125abf6dc6f5e.svg'),
	excelFileIcon: getUrl('/2024/10/19/606646a82b556132772944b60d55e62f.svg'),
	ppxtYellowFileIcon: getUrl('/2024/10/19/c84557261868a1bdbc986e647af81057.svg'),
	lunWenXueShuTranslate: getUrl('/2024/11/04/4ce4d21d5e6a19e6a8d34492e3f6118b.png'),
	enXueShuTranslate: getUrl('/2024/11/04/877b6d66324662f49987ca18989377f8.png'),
	AImusicBackIcon: getUrl('/2024/11/05/bfbfd79abe4f77f85f333c0a4cb353c4.jpeg'),
	AIMusicLogoIcon: getUrl('/2024/11/05/9d72e1ad3d1e9d29c8ff78ceb70a04b9.png'),
	AIZidongCreateSong: getUrl('/2024/11/12/96831f50d62913c197931153be265c4e.jpg'),
	AIMyCreateSong: getUrl('/2024/11/12/179450098adeb15e6daacfbf60ef214e.jpg'),
	todayIcon: getUrl('/2024/10/29/7b77f355e44fa2a4abc3dd76efca8784.png'),
	zhangJia: getUrl('/2024/10/29/b80fcc43555ad746c29a55935e14b230.png'),
	zhangzhi: getUrl('/2024/10/29/45e2ff326c72efe2487432f7af85d8b5.svg'),
	MusicSquare: getUrl('/2024/11/15/985b548bdf3dfaffd214530b90431445.png'),
	ImageCreateMusicIcon: getUrl('/2024/11/15/3e2d31d5e62cbd70e48f9edbb92b10bf.jpg'),
	VideoCreateMusicIcon: getUrl('/2024/11/15/30e658eb941270f7ba058e0786200031.jpg'),
	text2videoIcon: getUrl('/2024/11/16/29410bf626d2b551470b30e4b4829ef3.png'),
	createVideoVipIcon: getUrl('/2024/09/05/dbdf4cd56e183d831afc25fffd4c9376.svg'),
	wechatPayIcon: getUrl('/2024/09/03/261068293b2ed7c70be5846a5b5f3f91.svg'),
	alipayIcon: getUrl('/2024/09/03/edc2efc89e9d261fd7d494f1fd69c61d.svg'),
	image2videoIcon: getUrl('/2024/11/20/f9614e02f2d5f87837162d3269dc628f.gif'),
	AINumersPeopleBgImage: getUrl('/2024/11/21/a62446a8c386b72fb8407c7b5bc5e3e9.webp'),
	AIVideoHandlerImageBgImage: getUrl('/2024/11/21/8577b2c7f17ac3e5f9ba499415c7a00f.png'),
	AIExtractMusicImageBgImage: getUrl('/2024/11/21/0917b844709e0cd2f8b75579e43ec748.png'),
	AIWriteSomething: getUrl('/2024/11/25/79f9b0465fc54d3084bad803b2522622.jpg'),
	AIzhaiYaoIcon: getUrl('/2024/11/26/1cb63d4be54e007a555b701aef1fb941.jpg'),
	AIThesamePointIcon: getUrl('/2024/11/26/2b918abb903d71af4d6d67fbac3b95d1.jpg'),
	InspiredWritingIcon: getUrl('/2024/11/26/5eaea3601f86afd0a23da5e2fd1db85b.jpg'),
	articleWritingIcon: getUrl('/2024/11/27/7a7e0cd4e859bfe65d4232920eb432ec.png'),
	createTitleIcon: getUrl('/2024/11/27/84b05b286984a58870e6da1df9b03081.jpg'),
	behindQrcodeIcon: getUrl('/2024/08/28/efe303fb27f017f7e0ff68a40e303e8c.png'),
	editIcon: getUrl('/2024/03/05/33691d364176f3d543fb242ca0ab6eac.png'),
	articleQRcodoeIcon: getUrl('/2024/08/28/98ac051a564af614cfed5b6619b973a1.png'),
	aiAgentheaderModeIcon1: getUrl('/2024/11/30/c520e4a569d629e89ec37ed3400f7d28.jpeg'),
	aiAgentheaderModeIcon2: getUrl('/2024/11/30/7f7f88009f6a25e1b98ab055eb142fc4.jpeg'),
	aiAgentheaderModeIcon3: getUrl('/2024/11/30/28916e24b267dcbcc71fc368ed4d1784.jpg'),
	aiAgentheaderModeIcon4: getUrl('/2024/11/30/a72c3f5e29b67ab10ee8c550de942c51.jpg'),
	aiAgentheaderModeIcon5: getUrl('/2024/11/30/d92759f751ca936c16902d72d0921a09.jpeg'),
	aiAgentheaderModeIcon6: getUrl('/2024/11/30/ecbcd466e57610efe47202fbb39bafcb.jpg'),
	aiAgentheaderModeIcon7: getUrl('/2024/11/30/2469ae1cbebe4e0de16717818b24ac76.jpg'),
	aiAgentheaderModeIcon8: getUrl('/2024/11/30/5ba76bac0df96405e41215ac27e77a35.jpg'),
	aiAgentheaderModeIcon9: getUrl('/2024/12/02/6b146929a4bff5f713a1f828836295b0.jpg'),
	aiAgentheaderModeIcon10: getUrl('/2024/12/02/8ae5343561f7d5e3d069c291d9d3aae4.jpg'),
	aiAgentheaderModeIcon11: getUrl('/2024/12/02/42c83d44380b82c2711c4dec56b29dfa.jpg'),
	aiAgentheaderModeIcon12: getUrl('/2024/12/02/de4aeba01c79cabb062c64338a3ac8cd.jpeg'),
	aiAgentheaderModeIcon13: getUrl('/2024/12/02/339450ca423c9b37fcc97055b9fe9695.jpg'),
	aiAgentheaderModeIcon14: getUrl('/2024/12/02/1f02d83e2e6d714579523467b424ad63.png'),
	aiAgentheaderModeIcon: getUrl('/2024/12/02/f4be6379118c21871cec305a26346a54.jpeg'),
	uploadFileIcon: getUrl('/2024/11/15/06281018e267b6db9d8242258f3a81f9.svg'),
	aiAigentLoadingIcon: getUrl('/2024/12/05/1165fde6298d8426bfb5584e09a3abc6.gif'),
	aiAigentLoadingImage: getUrl('/2024/12/06/98c674deeb2da4cc48ab086d903d46d9.gif'),
	aiAigentDrawLoading: getUrl('/2024/12/06/d31346750914e8326dfa974ec58eaf92.gif'),
	aiRevers3D: getUrl('/2024/12/11/eaa59e8703fff4c61af0693b42c42e8c.png'),
	translate20: getUrl('/2024/12/12/1ecbc2151bf7fd25782b8356572e1125.jpg'),
	aiApplication: getUrl('/2024/12/13/d48c1073e68c148bea75cf7aed7fb9de.jpg'),
	Application2: getUrl('/2024/12/13/dbdf17d09e7835d8830608970dc04344.webp'),
	Application3: getUrl('/2024/12/13/f38f31474bd18255372f6d1dc3ef01e5.webp'),
	Application4: getUrl('/2024/12/13/0467e9a6051422bed62e4054eab406ce.webp'),
	Application5: getUrl('/2024/12/16/f04b277b9bcbdd5beffd721a0129ae53.webp'),
	Application6: getUrl('/2024/12/16/061657a1407bebbcb5c5f55a1db6c240.webp'),
	Application7: getUrl('/2024/12/16/6c3784f12f179640f3f23faa9bdfb286.webp'),
	Application8: getUrl('/2024/12/16/650d01d3b426bb8af35ad604ef283eee.webp'),
	Application9: getUrl('/2024/12/16/c05c55a56f96f11cfbc7b72eac0c644f.webp'),
	Application10: getUrl('/2024/12/16/d1e2a25768f06fad3bb2b177c163c447.webp'),
	Application11: getUrl('/2024/12/16/b5567ac778a58cf9dbff17cac1b5b261.jpg'),
	Application12: getUrl('/2024/12/16/fdd88669d1b380f901ccde1a0247e5b8.jpg'),
	Application13: getUrl('/2024/12/16/379a3557205be98a669254e8c73adaa4.jpg'),
	Application14: getUrl('/2024/12/16/19a87be12100040a44a327e938683a44.jpeg'),
	Application15: getUrl('/2024/12/16/a503e41727c857a77a38cda9e9945dde.webp'),
	Application16: getUrl('/2024/12/16/1cf26663b1197afbff55e2f3562727dc.webp'),
	Application17: getUrl('/2024/12/16/7b7f546e61897cf70be7e67c88f66f12.webp'),
	Application18: getUrl('/2024/12/16/ac85df12894ec8e78513705e1f2346e0.png'),
	Application19: getUrl('/2024/12/16/43d012413c248d5e41012ee9111978d7.webp'),
	Application20: getUrl('/2024/12/16/4c6f48c70cb8bfcef3afb5f33a50305a.jpg'),
	Application21: getUrl('/2024/12/16/5a0391361dfe96c9981176a5aa150a32.webp'),
	Application22: getUrl('/2024/12/16/a6e2703982045216ac68d903162b87ce.webp'),
	Application23: getUrl('/2024/12/16/790dfb2ff072194ebcb2ad37027fbdbf.jpg'),
	Application24: getUrl('/2024/12/16/80313b822a072ead218aeaf1131f798d.jpg'),
	ApplicationsBackground: getUrl('/2024/12/16/64639c24c88011dc33ebea90f16b8924.jpg'),
	NoApplicationsData: getUrl('/2024/12/17/318c0e6fd3d37e1f1a4fc6ecf0ecfb6c.png'),
	// deepseekModel icon 
	DeepSeekModelIcon: getUrl('/2025/01/02/df6570e38b9a23ce8aea1b4524b95bda.svg'),
	DeepSeekModalThinkIngIcon: getUrl('/2025/02/09/379662ddb132e683c9eb815021104d0a.svg'),
	copyIcon: getUrl('/2025/02/10/d6e0864517fd96415a5aef168d460fe2.svg'),
	reStartChatIcon: getUrl('/2025/02/10/8129facf60e8c17e5a49146ef71141fd.svg'),
	ExportIcon: getUrl('/2025/02/10/fa825ab103cde6b915c66bc66984d938.svg'),
	jubaoIcon: getUrl('/2025/02/10/854adb8db7fbc645a41b50a10c42073f.svg'),
	lianwangIcon: getUrl('/2024/07/13/347294b9cc60e5f510644999eda40c2a.png'),
	OverBGImage: getUrl('/2025/02/13/e47dece101d681759bae5a3aaa051192.png')

}

export default iconConfig